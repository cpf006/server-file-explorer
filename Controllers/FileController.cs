...
/* Handles browsing, searching, uploading and downloading files under a root folder. */
[ApiController]
[Route("api/files")]
public class FileController : ControllerBase
{
    ...
    public FileController(IOptions<FileExplorerOptions> options, ILogger<FileController> logger)
    {
        ...
        Directory.CreateDirectory(_root); // make sure the folder exists
        _logger = logger;
    }

    // Combine user input with the root folder and block attempts to escape it
    ...
    // Return folders and files at the requested path
    [HttpGet]
    ...
    // Upload a file to the requested folder
    [HttpPost("upload")]
    ...
    // Search all folders for names that contain the query
    [HttpGet("search")]
